...
with tab1:
    ...
    with st.expander("Positives & Yield"):
        filtered_df['Yield'] = (filtered_df['Positives'] / filtered_df['Total_tested'] * 100).replace([np.inf, -np.inf], 0)
        fig2 = go.Figure()
    ...
    with st.expander("ICT Testing Types"):
        df_long = filtered_df.melt(id_vars='Month',
